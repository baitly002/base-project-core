...
//            AbstractMessageConverterMethodProcessor //消息转换匹配器
//            StringHttpMessageConverter
            HttpMessageConverter converter = FastJsonHttpMessageConverterConfig.getConverterIstance();
        ...
        public static RequestContextFilter requestContextFilter() {
            return new WebOrderedRequestContextFilter();
//            return new OrderedRequestContextFilter();
    ...
    public static class EnableWebMvcConfiguration extends DelegatingWebMvcConfiguration implements ResourceLoaderAware {
        private static final Log logger = LogFactory.getLog(EnableWebMvcConfiguration.class);
        @Autowired
        private ApiRequestInterceptor apiRequestInterceptor;
        @Autowired
        private SecurityConfiger securityConfiger;
...
//            return super.createRequestMappingHandlerMapping();
            return new WebControllerHandlerMapping();
        }
        ...
        public void addInterceptors(InterceptorRegistry registry) {
            ...
            if(securityConfiger!=null){
                if(securityConfiger.getPaths()!=null && securityConfiger.getPaths().size()>0){
                    if(apiRequestInterceptor!=null){
                        interceptorRegistration = registry.addInterceptor(apiRequestInterceptor);
                    }else{
                        logger.error("拦截器没有初始化！");
                    }
                    if(interceptorRegistration != null) {
                        interceptorRegistration.addPathPatterns(securityConfiger.getPaths());
                    }
                }
                if(securityConfiger.getExcludes()!=null && securityConfiger.getExcludes().size()>0 && interceptorRegistration != null){
                    interceptorRegistration.excludePathPatterns(securityConfiger.getExcludes());
                }
//			registry.addInterceptor(apiRequestInterceptor).addPathPatterns(securityConfiger.getPaths()).excludePathPatterns(securityConfiger.getExcludes());
            }
//        registry.addInterceptor(apiRequestInterceptor).addPathPatterns("/**").excludePathPatterns("/gateway/login", "/gateway/test/**");
