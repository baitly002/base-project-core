 ...
 */
package org.basecode.common.generator.mybatis.generator.internal.util;
...
public class JavaBeansUtil {
    ...
    public static Field getJavaBeansField(IntrospectedColumn introspectedColumn,
        ...
        String comment = StringUtils.replaceBlank(introspectedColumn.getRemarks());
        String commentValue = "";
        String commentRemark = "";
        if(StringUtils.isNotBlank(comment)) {
        	int index = comment.indexOf(" ");
        	if(index>-1) {
        		commentValue = comment.substring(0, index);
        		commentRemark = comment.substring(index+1);
        	}else {
        		commentValue = comment;
        	}
        	
        }
        field.addAnnotation("@Doc(value=\""+commentValue+"\", remark=\""+commentRemark+"\")");
        if(introspectedColumn.getActualColumnName().equals("id")){
            field.addAnnotation("@TableId");
        }else {
            field.addAnnotation("@TableField");
        }
