...
import org.basecode.common.util.DateUtil;
...
public class Sequence {
	...
	private final long workerIdBits = 3L;/* 机器标识位数 */
	private final long datacenterIdBits = 1L;
	private final long maxWorkerId = -1L ^ (-1L << workerIdBits);
	private final long maxDatacenterId = -1L ^ (-1L << datacenterIdBits);
	private final long sequenceBits = 12L;/* 毫秒内自增位 12>4096 11>2460 10>1022*/ 
...
//		System.out.println("workerId:"+workerId);
	...
	public Sequence(long workerId, long datacenterId) {
		if (workerId > maxWorkerId || workerId < 0) {
			throw new BusinessException(String.format("worker Id can't be greater than %d or less than 0", maxWorkerId));
		}
	...
	public synchronized long nextId() {
		...
		if (timestamp < lastTimestamp) {
			throw new BusinessException(String.format("Clock moved backwards. Refusing to generate id for %d milliseconds",
	...
	public static void main(String[] args) {
		Date now = DateUtil.formatDate("2018-01-01");
		System.out.println(now.getTime());
		Sequence seq = new Sequence();
		System.out.println(seq.nextId());
		String s = seq.nextId()+"";
		System.out.println(s.length());
		//915946324316160 现在15位
		//到2060年17位
	}
}
